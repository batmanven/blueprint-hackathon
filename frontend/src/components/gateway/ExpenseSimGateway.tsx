...
import { Database, Search, ShieldCheck, Zap, Loader2, CheckCircle2, FileSearch, Scale, Calculator, LineChart, BrainCircuit } from 'lucide-react';

interface ExpenseSimGatewayProps {
  onComplete: () => void;
}

const ExpenseSimGateway = ({ onComplete }: ExpenseSimGatewayProps) => {
  const [step, setStep] = useState(0);
  const [done, setDone] = useState(false);

  const steps = [
    { icon: <Calculator size={18} />, label: 'Data Ingestion', text: 'Actuarial profile ingestion & validation...', duration: 1000 },
    { icon: <LineChart size={18} />, label: 'Inflation Modeling', text: 'Simulating local medical price indices...', duration: 1400 },
    { icon: <BrainCircuit size={18} />, label: 'Scenario Engine', text: 'Running 10,000+ critical health simulations...', duration: 1600 },
    { icon: <ShieldCheck size={18} />, label: 'Risk Verdict', text: 'Finalizing Shield target & buffer metrics...', duration: 800 },
  ];
        ...
        <div className="px-10 pt-10 pb-8 flex flex-col items-center gap-5 text-center">
          <div className="relative">
            <div className="h-20 w-20 bg-fin/10 rounded-3xl flex items-center justify-center">
              ...
              />
              <BrainCircuit className="h-9 w-9 text-fin relative z-10" />
            </div>
          </div>
          <div>
            <h2 className="text-[22px] font-semibold text-off-black">Actuarial Simulation</h2>
            <p className="text-[13px] text-black/40 mt-1 font-medium">Medical Financial Intelligence Unit · v2.4.0-PEAK</p>
          </div>
        </div>
        ...
        <div className="px-10 py-5 bg-[#faf9f6] border-t border-oat flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-bold text-black/30 uppercase tracking-[0.2em]">
              {done ? 'Simulation Complete' : 'AI Simulation Active'}
            </span>
          </div>
          <span className="text-[11px] font-bold text-black/20 uppercase tracking-[0.18em]">NPPA · NHA.GOV</span>
        </div>
      </motion.div>
    </div>
  );
...
export default ExpenseSimGateway;
