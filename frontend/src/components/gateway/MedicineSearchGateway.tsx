...
import { Database, Search, ShieldCheck, Zap, Loader2, CheckCircle2 } from 'lucide-react';

interface MedicineSearchGatewayProps {
  query: string;
  onComplete: () => void;
}

const MedicineSearchGateway = ({ query, onComplete }: MedicineSearchGatewayProps) => {
  const [step, setStep] = useState(0);
  const [done, setDone] = useState(false);

  const steps = [
    { icon: <Search size={18} />, label: 'Brand Analysis', text: `Scanning registry for "${query}"...`, duration: 900 },
    { icon: <Database size={18} />, label: 'PMBJP Registry', text: 'Connecting to Jan Aushadhi National Registry...', duration: 1200 },
    { icon: <ShieldCheck size={18} />, label: 'Bio-equivalence', text: 'Validating salt composition & efficacy data...', duration: 1000 },
    { icon: <Zap size={18} />, label: 'Cost Intelligence', text: 'Calculating DPI savings potential...', duration: 700 },
  ];
        ...
        <div className="px-10 pt-10 pb-8 flex flex-col items-center gap-5 text-center">
          <div className="relative">
            <div className="h-20 w-20 bg-fin/10 rounded-3xl flex items-center justify-center">
              ...
              />
              <Database className="h-9 w-9 text-fin relative z-10" />
            </div>
          </div>
          <div>
            <h2 className="text-[22px] font-semibold text-off-black">Aarogya Intelligence</h2>
            <p className="text-[13px] text-black/40 mt-1 font-medium">Pharmacological Optimization Engine · v2.4.0-PEAK</p>
          </div>
        </div>
        ...
        <div className="px-10 py-5 bg-[#faf9f6] border-t border-oat flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-bold text-black/30 uppercase tracking-[0.2em]">
              {done ? 'Analysis Complete' : 'Secure DPI Bridge Active'}
            </span>
          </div>
          <span className="text-[11px] font-bold text-black/20 uppercase tracking-[0.18em]">PMBJP · ABDM</span>
        </div>
      </motion.div>
    </div>
  );
...
export default MedicineSearchGateway;
