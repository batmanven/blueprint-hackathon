...
import { Database, Search, ShieldCheck, Zap, Loader2, CheckCircle2, FileSearch, Scale, AlertTriangle, FileText } from 'lucide-react';

interface BillAuditGatewayProps {
  onComplete: () => void;
}

const BillAuditGateway = ({ onComplete }: BillAuditGatewayProps) => {
  const [step, setStep] = useState(0);
  const [done, setDone] = useState(false);

  const steps = [
    { icon: <FileText size={18} />, label: 'OCR Extraction', text: 'Scanning bill image & itemizing charges...', duration: 1200 },
    { icon: <FileSearch size={18} />, label: 'Audit Scan', text: 'Detecting unbundled & inflated line items...', duration: 1500 },
    { icon: <Scale size={18} />, label: 'NPPA Benchmarking', text: 'Cross-referencing against Govt. Price caps...', duration: 1300 },
    { icon: <AlertTriangle size={18} />, label: 'Inflation Detect', text: 'Finalizing potential savings verdict...', duration: 900 },
  ];
        ...
        <div className="px-10 pt-10 pb-8 flex flex-col items-center gap-5 text-center">
          <div className="relative">
            <div className="h-20 w-20 bg-fin/10 rounded-3xl flex items-center justify-center">
              ...
              />
              <FileSearch className="h-9 w-9 text-fin relative z-10" />
            </div>
          </div>
          <div>
            <h2 className="text-[22px] font-semibold text-off-black">Aarogya Audit</h2>
            <p className="text-[13px] text-black/40 mt-1 font-medium">Healthcare Billing Integrity Engine · v2.4.0-PEAK</p>
          </div>
        </div>
        ...
        <div className="px-10 py-5 bg-[#faf9f6] border-t border-oat flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-bold text-black/30 uppercase tracking-[0.2em]">
              {done ? 'Audit Complete' : 'AI Integrity Logic Active'}
            </span>
          </div>
...
export default BillAuditGateway;
